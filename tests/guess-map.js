...
var Backbone = require('./backbone-fake');
global._ = require('underscore'); // Implicit dependency for guessMap

// Shared for all tests
var sql = 'SELECT * FROM testable';

test('date column', function(t) {
  t.plan(1);

  var column = new Backbone.Model({
    name: 'object_postedtime',
    type: 'date',
    geometry_type: 'point',
    bbox: [
  ...
  var results = Pecan.guessMap(sql, 'testable', column, stats);
...
test('number column', function(t) {
  ...
  results = Pecan.guessMap(sql, 'testable', column, stats);
  ...
  column.set('geometry_type', 'polygon');
  results = Pecan.guessMap(sql, 'testable', column, stats);
...
test('string column', function(t) {
  ...
  // Treat as string for syntax parsers to not go haywire…
